...
const SERVICES = [
  {
    ...
    category: "polisi",
    categoryLabel: "Polisi",
    address: "Jl. M. Yamin, Kota Padang",
    phone: "110",
    latitude: -0.9497,
    longitude: 100.3596
  },
  {
    ...
    address: "Jl. Perintis Kemerdekaan, Kota Padang",
    phone: "(0751) 32371",
    latitude: -0.9424,
    longitude: 100.3672
  },
  {
    id: 3,
    name: "UPTD Perlindungan Perempuan dan Anak",
    category: "konseling",
    categoryLabel: "Pendampingan",
  ...
  {
    id: 4,
    name: "Puskesmas Padang Pasir",
    category: "kesehatan",
    categoryLabel: "Kesehatan",
    address: "Jl. Padang Pasir, Kota Padang",
    phone: "(0751) 893212",
    latitude: -0.9451,
    longitude: 100.3544
  },
  {
    id: 5,
    name: "Lembaga Bantuan Hukum Padang",
    category: "hukum",
...
const categoryButtons = document.querySelectorAll(
  "#category-buttons [data-category]"
);
...
function calculateDistance(firstPosition, service) {
  ...
  const latitudeDifference = toRadians(
    service.latitude - firstPosition.latitude
  );

  const longitudeDifference = toRadians(
    service.longitude - firstPosition.longitude
  );
  ...
  return SERVICES
    .filter(function (service) {
      ...
      const searchableValue =
        `${service.name} ${service.address}`.toLowerCase();

      const matchesQuery = searchableValue.includes(query);
    ...
    .map(function (service) {
      return {
        ...service,
        distance: calculateDistance(userPosition, service)
      };
    })
    .sort(function (first, second) {
      return (first.distance ?? 9999) -
        (second.distance ?? 9999);
    });
}
...
function renderServices() {
        ...
        >
          <div class="service-card-header">
            ...
            <span class="service-category">
              ${escapeHTML(service.categoryLabel)}
            </span>
          </div>
          ...
          <div class="service-information">
            ...
            <a
              href="tel:${createPhoneURL(service.phone)}"
  ...
  services.forEach(function (service) {
    ...
    marker.bindPopup(`
      ...
      ${escapeHTML(service.address)}
      <br><br>
      <a href="tel:${createPhoneURL(service.phone)}">
        Hubungi ${escapeHTML(service.phone)}
      </a>
    `);
...
locateButton.addEventListener("click", function () {
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    function () {
      ...
      window.alert(
        "Lokasi tidak dapat diakses. Berikan izin lokasi pada browser."
      );
    },
    {
